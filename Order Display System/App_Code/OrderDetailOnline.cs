...
namespace KOTPrintUtility.App_Code
{
	public class OrderRequestModel
	{
		...
		public List<string> Zomato_order_id { get; set; }
	...
	{
		public string bill_no_WebOrder { get; set; }
		public string bill_no_local { get; set; }
  ...
  string subTotal, string Bill_DiscountPct, string dept_head, string OrderNo, string Other_Charge, tbl_bill Objbill, out string Bill_No,
		...
		public async Task<BillRespose> GenerateBill_HDAPI(clsOrdersBill dsBillDetail, string POS_code, string Bill_Type, string Bill_Date, string Bill_Amount,
			   string Dis_Amount, string Service_Charge_Amount, string Paid_Amount, string Balance, string Payment_Mode, string Card_No, string No_of_Item, string To_Whom,
			   string Comments, string Cashier, string tax1, string roundoff, string Discount_by, string TockenNo, string Service_Charge, string sur_charge, string Service_Tax,
			   string HoldBill_No, string cardType, string cardAmount, string OutletId, string subTotal, string Bill_DiscountPct, string dept_head, string OrderNo, string Other_Charge,
			   tbl_bill Objbill, string RegisterUserStatus, string SBC_Tax, string KKC_Tax, string order_status, string IsBillSattle,
		 string weborderCommnet, string RedeemPoints, string lblRedeemPoints, string is_instant_order = "0")
		{
			BillRespose response = new BillRespose();
			response.Result = false;
			response.Bill_no = string.Empty;
			string sql_Coupon = string.Empty;
			string hostName = Dns.GetHostName();
			string ipAddress = Dns.GetHostByName(hostName).AddressList[0].ToString();
			int result = 0;
			...
			try
			{
				...
				if (result > 0)
				{
					...
					// execute quuery :insert in tbl_assorted_item
					string sqlAssorted = string.Empty;
					for (int i = 0; i < dsBillDetail.combos.Count; i++)
					{
						string i_code = dsBillDetail.combos[i].i_code;
						string i_code_fk = dsBillDetail.combos[i].i_code_fk;
						double Qty = Convert.ToDouble(dsBillDetail.combos[i].Qty);
						string Order_Type = dsBillDetail.combos[i].Order_Type;
						if (Order_Type == "")
							Order_Type = "A";
						string item_index = dsBillDetail.combos[i].ItemIndex;
						string isDiscount = dsBillDetail.combos[i].IsDiscount;
						//string Index_No = ""
						string Index_No = "0";
						string DishComment = dsBillDetail.combos[i].DishComment;
						string ComboAmt = dsBillDetail.combos[i].amount.ToString();
						if (ComboAmt == "")
							ComboAmt = "0";
						double ComboRate = Convert.ToDouble(ComboAmt) / Qty;
					...
					//bool ResultobjOnline = true;
					bool ResultobjOnline = await clsUpdateOrder.UpdatOrderStatusOnlineCloudDBAPI(Objbill.bill_no_WebOrder, LastBillNo, "Confirm", Objbill.Channel.ToLower());
		...
		public async Task<Cls_OnlineOrder> GetOnlineOrderFromApiAsync()
		{
			Cls_OnlineOrder order = new Cls_OnlineOrder();
			try
			{
				using (HttpClient client = new HttpClient())
				{
					...
					//using (HttpClient client = new HttpClient())
					//{
					//	//string apiUrl = "https://urbanswiggyzomatoapi.azurewebsites.net/api/cloud/GetOrderinPOSAndUtility";

					//	string bill_date = Program.DayEnd_BIllingDate;
					//	string outlet_id = Program.Outlet_id;
					//	string zomato_order_idss = GetAllPunchedZomatoIds();
					//	int restaurant_id = 0;
					//	List<string> Zomato_order_id = new List<string> { zomato_order_idss };

					//	string ids = string.Join(",", Zomato_order_id);  // Combine the list into one string

					//	// URL-encode parameters
					//	string fullUrl = $"{clsUpdateOrder.GetConfigUrl("GetCloudorder").url}?" +
					//					 $"bill_date={WebUtility.UrlEncode(bill_date)}&" +
					//					 $"outlet_id={WebUtility.UrlEncode(outlet_id)}&" +
					//					 $"Zomato_order_id={WebUtility.UrlEncode(ids)}&" +
					...
					//		"ARRAffinity=25be61639a184fe3469f0e787307f5070870c561aa0008ca52aef91e9ac255e7;" +
					//		"ARRAffinitySameSite=25be61639a184fe3469f0e787307f5070870c561aa0008ca52aef91e9ac255e7");

					//	var response = await client.SendAsync(request);
					//	response.EnsureSuccessStatusCode();
					//	string responseContent = await response.Content.ReadAsStringAsync();
					//	order = JsonConvert.DeserializeObject<Cls_OnlineOrder>(responseContent);
					//	//js.Deserialize<WebhookResponse>(apiRetrun.JsonString);
					//}
				}
			}
			catch (Exception ex)
